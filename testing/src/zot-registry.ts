...
export const ZOT_IMAGE =
  'ghcr.io/project-zot/zot-minimal@sha256:095d619a8860e0f9eee01dd53a58320f2f94e4aa1525e6ea75dddff3518f567f';
...
export class ZotRegistry {
  ...
  async start(): Promise<void> {
    ...
    // Start Zot registry
    execSync(
      `${this._runtime} run -d --name ${this._containerName} -p ${this._port}:5000 ${ZOT_IMAGE}`,
      {
        stdio: 'inherit',
      },
    );
  ...
  async stop(): Promise<void> {
    ...
    const shouldKeep =
      process.env.KEEP_ZOT_REGISTRY === '1' || process.env.KEEP_ZOT_REGISTRY === 'true';
...
export function shouldSkipZotTests(): boolean {
  ...
  if (!runtime) {
    ...
    if (requireTests) {
      throw new Error(
        'REQUIRE_PUSH_TESTS is set but no container runtime (podman/docker) is available',
      );
    }
