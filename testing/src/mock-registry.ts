...
const AMD64_CONFIG = Buffer.from(
  JSON.stringify({
    architecture: 'amd64',
    os: 'linux',
    config: {},
...
const AMD64_MANIFEST_BUF = Buffer.from(
  JSON.stringify({
    schemaVersion: 2,
...
const ARM64_CONFIG = Buffer.from(
  JSON.stringify({
    architecture: 'arm64',
    os: 'linux',
    config: {},
...
const ARM64_MANIFEST_BUF = Buffer.from(
  JSON.stringify({
    schemaVersion: 2,
    mediaType: 'application/vnd.oci.image.manifest.v1+json',
    config: {
...
const IMAGE_INDEX_BUF = Buffer.from(
  JSON.stringify({
    schemaVersion: 2,
...
async function generateTlsCerts(): Promise<TlsCerts> {
  ...
  const pems = await selfsigned.generate([{ name: 'commonName', value: 'Mock Test CA' }], {
    notAfterDate,
    keySize: 2048,
    algorithm: 'sha256',
    extensions: [
